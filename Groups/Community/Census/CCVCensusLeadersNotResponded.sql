...
INNER JOIN [Group] AS g ON gt.Id = g.GroupTypeId
    AND gt.Id = 49
    AND g.IsActive = 1
    AND g.IsArchived != 1
INNER JOIN [Group] coachg ON g.ParentGroupId = coachg.Id
 AND coachg.IsActive = 1 AND coachg.IsArchived != 1
INNER JOIN [Group] slg ON coachg.ParentGroupId = slg.Id
    AND slg.ParentGroupId = 26516
    AND slg.IsActive = 1 AND slg.IsArchived != 1
    AND slg.ParentGroupId = 26516
LEFT JOIN [AttributeValue] AS av ON g.Id = av.EntityId
    AND av.AttributeId = 6309
LEFT JOIN AttributeMatrix AS am ON av.[Value] = TRY_CAST(am.Guid AS NVARCHAR(50))
OUTER APPLY(
SELECT ctdf.Value 
FROM AttributeMatrixItem AS cami
INNER JOIN AttributeValue AS cqav ON cami.Id = cqav.EntityId
...
WHERE NOT EXISTS (
        SELECT 1
        FROM Workflow w
        INNER JOIN AttributeValue av ON w.Id = av.EntityId
    ...
    AND NOT EXISTS (
        ...
        INNER JOIN Attribute ap ON avp.AttributeId = ap.Id
            AND ap.[Key] = 'Person'
        INNER JOIN GroupMember gm2 ON avp.ValueAsPersonId = gm2.PersonId
            AND gm2.Id = gm.Id
        INNER JOIN [Group] g ON gm2.GroupId = g.Id
            AND av.[Value] = g.Guid
        )
    AND NOT EXISTS (
        SELECT 1
        FROM GroupType AS gt
        INNER JOIN [Group] AS g2 ON gt.Id = g2.GroupTypeId
            AND gt.Id = 49
            AND g2.Id = g.Id
        INNER JOIN [AttributeValue] dav ON g2.Id = dav.EntityId
            AND dav.AttributeId = 5920
            AND dav.[Value] != 'Students'
        INNER JOIN [AttributeValue] AS av ON g2.Id = av.EntityId
            AND av.AttributeId = 6309
        INNER JOIN AttributeMatrix AS am ON av.[Value] = convert(NVARCHAR(50), am.Guid)
        INNER JOIN AttributeMatrixItem AS cami ON am.Id = cami.AttributeMatrixId
        INNER JOIN AttributeValue AS cqav ON cami.Id = cqav.EntityId
            AND cqav.AttributeId = 6312
        INNER JOIN AttributeValue AS ctav ON cami.Id = ctav.EntityId
            AND ctav.AttributeId = 6307
